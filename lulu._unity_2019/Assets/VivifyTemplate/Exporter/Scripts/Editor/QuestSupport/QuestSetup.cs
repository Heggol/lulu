        ...
        private bool EditorChecks()
        {
            if (!Directory.Exists(Path.GetDirectoryName(QuestPreferences.UnityHubPath)))
            {
                EditorGUILayout.LabelField(
                    "You do not have Unity Hub installed. This is required for managing your editor installations.",
            ...
            if (State == BackgroundTaskState.Idle && (QuestPreferences.UnityEditor == "" ||
                                                      !Directory.Exists(
                                                          Path.GetDirectoryName(QuestPreferences.UnityEditor))))
            {
                if (HubWrapper.TryGetUnityEditor("2021.3.16f1", out var foundVersion) &&
                    Directory.Exists(Path.GetDirectoryName(foundVersion)))
                {
                ...
                else
                {
                    ...
                    EditorGUILayout.LabelField(
                        ...
                        style, GUILayout.Height(style.fontSize * 2));
                    EditorGUILayout.Space(10);
                    ...
                    EditorGUILayout.Space(10);
                    EditorGUILayout.LabelField(
                        "Make sure you check the box that says \"Android Build Support\" when installing.", style,
                        GUILayout.Height(style.fontSize * 2));
                    EditorGUILayout.Space(10);
...
#elif UNITY_EDITOR_LINUX
            bool isUnityEditorInstalled = File.Exists(Path.Combine(QuestPreferences.UnityEditor));
#endif
